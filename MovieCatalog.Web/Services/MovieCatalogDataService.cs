...
public class MovieCatalogDataService : IMovieCatalogDataService
{
    ...
    {
        var genres = await ctx.Genres
            .Include(g => g.TitleGenres)
            .ThenInclude(tg => tg.Title)
            .ToListAsync();

        var titleCounts = new Dictionary<Genre, int>();
        foreach (var genre in genres)
        {
            var count = genre.TitleGenres.Count;
            titleCounts[genre] = count;
        }
        return titleCounts.OrderBy(g => g.Key.Name).ToDictionary(g => g.Key, g => g.Value);
    }
